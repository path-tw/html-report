...
function delayBooks (grouped,booksContainer, loadBooks,button) {
...
const displaypopup = function () {
  const alert = 'please wait books are still loading';
  const popbox = document.createElement('p');
  popbox.className = 'loadBooks';
  const main = document.getElementById('main');
  popbox.innerText = alert;
  const body = document.getElementsByTagName('body')[0];
  body.style.backgroundColor = 'grey';
  main.appendChild(popbox);
  return new Promise((resolve, reject) => {
    setTimeout(()=>{
      popbox.remove();
...
const renderGroupedBooks = (groupSelection) => {
  ...
  if (check || groupSelection == 'none') {
    return new Promise((resolve, reject) => { 
      if (check === false && groupSelection == 'none') {
        setTimeout(()=>{
        delayBooks(grouped,booksContainer, loadBooks,button);
        check = true;
      }, 5000);
      } else {
        delayBooks(grouped,booksContainer, loadBooks,button);
