...
// const temporaryPopup = (duration = 2000) => {
//   const pTagData =[createPTag('Books are still loading, please wait...')];
//   const popup = createPopup( '', pTagData, 'ok', () => closePopup(popup));
//   showPopup(popup);
//   setTimeout(() => closePopup(popup), duration);
// };
...
const temporaryPopup = () => {
  ...
  popUpDivTag.innerText = 'Books are still loading, please wait...';
...
const createDropdown = (text, scrollToTop, parent) => {
  ...
  select.addEventListener('change', () => {
    const loadingMessage = document.getElementById('books').innerHTML.includes('Loading books...');
    if (loadingMessage) {
      temporaryPopup();
      const groupBy = document.querySelector('#group-by');
      groupBy.value = 'none';
    } else {
      groupAndRenderBooks(scrollToTop);
    }
  });
...
const render = () => {
  ...
  loadingMessage.style.marginTop = '40vh';
