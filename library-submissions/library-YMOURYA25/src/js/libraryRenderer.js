...
const createButton = (text, className, callback) => {
  ...
  callback && setSelectEventListeners(button,callback)
  return button;
...
const createDropdown = (text, scrollToTop, parent) => {
  ...
  select.id = id;
  select.name = id;
  setSelectEventListeners(select, groupAndRenderBooks,scrollToTop,"change");
...
const setSelectEventListeners = (elements, callback,parametre,event = 'click') => {
  elements.addEventListener(event, createPopUpPageLoadingAndDeleting);

  setTimeout(() => {
    elements.removeEventListener(event, createPopUpPageLoadingAndDeleting);
    elements.addEventListener(event, () => callback(parametre));
  }, 5000);
};
...
const addHomePageActions = () => {
  ...
  return new Promise((resolve,reject) =>{
    resolve("add home page actions resolved");
  })
};
...
const renderGroupedBooks = async (groupSelection) => {
  ...
  for (const [groupedOption, books] of grouped) {
      ...
      for (let index = 0; index < books.length; index++) {
        renderBook(books[index], fieldSet);
...
const pageLoadPopUp = () => {
  const existingPopup = document.getElementById('load-popup');
  if (!existingPopup) {
    const popup = document.createElement('div');
    popup.innerText = 'Wait! Page is still loading';
    const main = document.getElementById('main');
    popup.id = 'load-popup';
    popup.className = 'load-popup';
    main.appendChild(popup);
    return new Promise((resolve) => {
      resolve('page is popuped');
    });
  } 
};
...
const createPopUpPageLoadingAndDeleting = () => {
  const existingPopup = document.getElementById('load-popup-loading');
  if (!existingPopup) {
    const popup = document.createElement('div');
    popup.innerText = 'page is still loading wait until page is loaded';
    const main = document.getElementById('main');
    popup.id = 'load-popup-loading';
    popup.className = 'load-popup-loading';
    main.appendChild(popup);
    setTimeout(() => {
      popup.remove();
    }, 2000);
