const handleGroupDropdownClick = () => groupAndRenderBooks();
const handleAddBookButtonClick = () => showAddBookPopup();
...
const createDropdown = (text, scrollToTop, parent) => {
  ...
  select.id = id;
  select.name = id;
  select.addEventListener('change', handleGroupDropdownClick);
...
const createAndAppendAddBookButton = (parent) => {
  ...
  const addBookButton = createButton(text, className, handleAddBookButtonClick);
...
const setEventListeners = (action) => {
  const groupDropdown = document.getElementById('group-by');
  const addBookButton = document.getElementsByClassName('add-book-button')[0];

  if (action === 'addLoading') {
    groupDropdown.addEventListener('change', showBooksLoadingPopup);
    addBookButton.addEventListener('click', showBooksLoadingPopup);
  } else if (action === 'removeLoading') {
    groupDropdown.addEventListener('change', handleGroupDropdownClick);
...
const removeEventListeners = () => {
  ...
  groupDropdown.removeEventListener('change', handleGroupDropdownClick);
  addBookButton.removeEventListener('click', handleAddBookButtonClick);
  
  groupDropdown.removeEventListener('change', showBooksLoadingPopup);
  addBookButton.removeEventListener('click', showBooksLoadingPopup);
};
...
const showLoadingIndicator = () => {
  ...
  const legend = document.createElement('legend');
  legend.innerText = 'Books Loading..';
  loaderContainer.appendChild(legend);
...
const showBooksLoadingPopup = () => {
  if (document.getElementById('loading-popup')) return;
  ...
  message.innerText = 'Books are still loading... Please wait.';
  loadingPopup.appendChild(message);
  
  document.body.appendChild(loadingPopup);
  loadingPopup.showModal();

  document.getElementById('group-by').selectedIndex = 0;
  setTimeout(() => {
    hideBooksLoadingPopup();
  }, 2000);
};

const hideBooksLoadingPopup = () => {
