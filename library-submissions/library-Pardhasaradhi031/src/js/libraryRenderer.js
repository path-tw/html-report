...
const createDropdown = (text, scrollToTop, parent) => {
  ...
  select.id = id;
  select.name = id;
  select.addEventListener('change', handler = function () {
    return groupAndRenderBooks(scrollToTop);
  });
...
const createAndAppendAddBookButton = parent => {
  ...
  const addBookButton = createButton(text, className, showAddBookForm = () => showAddBookPopup());
