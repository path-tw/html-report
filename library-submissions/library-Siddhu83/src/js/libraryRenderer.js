...
const displayWaitPopup = () => {
  const waitMessage = createPTag('please wait! the books are loading..', 'wait-message');
  const waitPopup = createPopup('Note', [waitMessage]);
  showPopup(waitPopup);
  setTimeout(() => closePopup(waitPopup), 2000);
};

const createDropdown = (text, parent) => {
  ...
  select.id = id;
  select.name = id;
  select.addEventListener('change', displayWaitPopup);
...
const groupAndRenderBooks = async (scrollToTop = false) => {
...
const displayLoader = async () => {
  const loadMessage = createPTag('Books are Loading wait for few moments...', 'load-message');
  const booksContainer = document.getElementById('books');
  booksContainer.innerHTML = '';
  booksContainer.appendChild(loadMessage);

  return new Promise((resolve, reject) => {
    setTimeout(() => {
      resolve();
      removeExistingWaitPopup();
    }, 5000);
  });
};

const changeListenersOfHomeActions = () => {
  var dropDown = document.getElementById('group-by');
  dropDown.removeEventListener('change', displayWaitPopup);
  dropDown.addEventListener('change', () => groupAndRenderBooks(true));
  dropDown.value = 'none';
  const addBookButton = document.getElementsByClassName('add-book-button')[0];
  addBookButton.removeEventListener('click', displayWaitPopup);
