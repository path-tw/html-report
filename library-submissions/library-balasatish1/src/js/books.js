...
const showAlertPopup = function () {
  const alertPopup = document.querySelector('.loading-popup');
  alertPopup.style.cssText = `visibility: visible;
  animation-name: open-popup-animation;
  animation-duration: 200ms;
  animation-timing-function: ease-out;
  animation-fill-mode: forwards;`;
  const animationTimeout = setTimeout(() => {
    alertPopup.style.animationName = 'close-popup-animation';
    alertPopup.style.animationDuration = '200ms'
  }, 2000);
  return animationTimeout;
};

const setEventWhileLoading = () => {
  const loadingContainer = document.querySelector('#loading-container');
  let animationTimeout = null;
  loadingContainer.addEventListener('click', (clickEvent) => {
    if (clickEvent.target.id === 'loading-container') {
      clearTimeout(animationTimeout);
      animationTimeout = showAlertPopup();
    }
  });
};

const showLoadingAnimation = async function () {
  setEventWhileLoading();
  const loadingContainer = document.querySelector('#loading-container');
  const alertPopup = document.querySelector('.loading-popup');
  return new Promise ((resolve, reject) => {
    setTimeout(() => {
        ...
        alertPopup.remove();
        resolve('loading of Books Completed !!');
    }, 5000);
  });
};

const loadAndRenderBooks = async function () {
