...
const intializeBookProcessing = async function (status = false) {
  return new Promise(function (resolve, reject) {
    if (status) {
      resolve(groupAndRenderBooks());
    } else {
      reject("PLEASE WAIT ");
    }
  });
...
const attachClickHandlers = function () {
  const homeActionButtons = document.getElementsByClassName("home-actions");
  Array.from(homeActionButtons).forEach((button) => {
    button.addEventListener("click", handleStatus);
  });
};

const handleStatus = function (element) {
  displayCautionMessage();
  setTimeout(hideCautionMessage, 2000);
};

const displayCautionMessage = async function () {
  const cautionElement = document.getElementById("caution");
  let loadingStatus = false;
  if (!loadingStatus) {
    cautionElement.innerText = "Please wait...";
    // document.getElementById("group-by").style.display = "none";
  }
  loadingStatus = await loading();
  if (loadingStatus) {
...
const hideCautionMessage = function () {
  const cautionElement = document.getElementById("caution");
  cautionElement.style.display = "none";
};
 const displayCategorise = function (){
 let dropDown =  document.getElementById("group-by");
...
const render = async () => {
  ...
  await intializeBookProcessing(result).then(() => {
