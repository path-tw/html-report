...
const addHomePageActionsAndLoader = () => {
    ...
    let loader = booksLoader(mainConatiner);
    return new Promise((resolve, reject) => {
    setTimeout(() => {
      mainConatiner.removeChild(loader);
      resolve = (groupAndRenderBooks() === true) ? 'true' : 'false';
    }, 5000);
  });
};

const booksLoader = function (mainConatiner) {
  const loader = document.createElement('p');
  loader.classList.add('loader');
  loader.innerText = 'Loading please wait...';
  mainConatiner.appendChild(loader);
  return loader;
}
...
const render = () => {
  addHomePageActionsAndLoader().then(value => {console.log(value);});
