...
const showLoaderPopup = () => {
  const popPupDivison = document.createElement('div');
  popPupDivison.className = 'loadPopup';
  popPupDivison.innerText = 'Books are still Loading!!! Please Wait...';
  document.body.appendChild(popPupDivison);
  setTimeout(() => {
    popPupDivison.remove();
  }, 2000);
};
...
const createDropdown = (text, scrollToTop, parent) => {
  ...
  select.addEventListener('change', () => {
    const state = document.querySelector('#books').innerText.includes('Loading Books....');
    state ? showLoaderPopup() : groupAndRenderBooks(scrollToTop);
  });
...
const createAndAppendAddBookButton = parent => {
  ...
  const addBookButton = createButton(text, className, () => {
    const state = document.querySelector('#books').innerText.includes('Loading Books....');
    state ? showLoaderPopup() : showAddBookPopup();
  });
...
const createLoadingDivison = () => {
  const booksContainer = document.querySelector('#books');
  const loadMessage = document.createElement('div');
  loadMessage.textContent = 'Loading Books....';
