...
const alertMessage = () => {
  ...
  alertDiv.innerText = 'Books are still loading...';
  mainDiv.append(alertDiv);
  setTimeout(() => {
...
const createAndAppendAddBookButton = parent => {
  ...
  const addBookButton = createButton(text, className, handleAddBook);
...
const handleDropdown = () => groupAndRenderBooks();
const handleAddBook = () => showAddBookPopup();
const handleLoadingAlert = () => alertMessage();

const loadingBooks = () => {
  const groupDropdown = document.getElementById('group-by');
  const HandleAddBookButton = document.getElementsByClassName('add-book-button')[0];
  const mainDiv = document.getElementById('main');
  const loadingText = document.createElement('div');
  loadingText.id = 'loadingText';
  loadingText.innerText = 'Loading....';
  mainDiv.append(loadingText);
  groupDropdown.removeEventListener('change', handleDropdown);
  HandleAddBookButton.removeEventListener('click', handleAddBook);

  groupDropdown.addEventListener('change', handleLoadingAlert);
  HandleAddBookButton.addEventListener('click', handleLoadingAlert);
};

const hideLoading = () => {
  const groupDropdown = document.getElementById('group-by');
  const HandleAddBookButton = document.getElementsByClassName('add-book-button')[0];
  const loadBooks = document.getElementById('loadingText');
  loadBooks.style.display = 'none';
  groupDropdown.removeEventListener('change', handleLoadingAlert);
  HandleAddBookButton.removeEventListener('click', handleLoadingAlert);
  groupDropdown.addEventListener('change', handleDropdown);
  HandleAddBookButton.addEventListener('click',handleAddBook);
};
