...
const showLoadingMessage = (message) => {
  if (!loadingMessageElement) {
    ...
    loadingMessageElement.style.backgroundColor = 'rgba(0, 0, 0, 0.7)';
    loadingMessageElement.style.color = '#fff';
    loadingMessageElement.style.zIndex = '1000';
...
const loadBooks = () => {
  return new Promise((resolve) => {
    isLoading = true;
    showLoadingMessage('Loading books, please wait...');
...
const handleHomeActionClick = () => {
  if (isLoading) {
    showLoadingMessage('Books are being loaded, please wait...');
  }
};
...
const addHomePageActions = () => {
  ...
  const groupDropdown = document.getElementById('group-by');
  groupDropdown.addEventListener('change', handleHomeActionClick);

  const featureButtons = document.querySelectorAll('.feature-button');
  featureButtons.forEach(button => {
    button.addEventListener('click', handleHomeActionClick);
  });

  const header = document.getElementById('header');
  header.addEventListener('click', handleHomeActionClick);
};

const render = () => {
  addHomePageActions();
  loadBooks().then(() => {
    groupAndRenderBooks(false);
  });
};
