...
const booksLoadedOrNot = (callback1, callback2, scrollToTop) => {
  if (document.querySelector('.books-group')) {
    callback1(scrollToTop);
  } else {
...
const createDropdown = (text, scrollToTop, parent) => {
  ...
  select.id = id;
  select.name = id;
  select.addEventListener('change', () => booksLoadedOrNot(groupAndRenderBooks, notAllowedPopUp, scrollToTop));
...
const createAndAppendAddBookButton = parent => {
  ...
  const addBookButton = createButton(text, className, () => {
    booksLoadedOrNot(showAddBookPopup, notAllowedPopUp);
  });
...
const createNotAllowedPopUp = () => {
  const popupDiv = document.createElement('div');
  popupDiv.classList.add('not-allowed-popup');
  popupDiv.textContent = 'This Operation is not Allowed. \n Please wait until books loaded';
  return popupDiv;
};
...
const notAllowedPopUp = () => {
  const booksContainer = document.querySelector('#books');
  const popupBox = createNotAllowedPopUp();
  booksContainer.appendChild(popupBox);
  setTimeout(() => {
    hidePopup(popupBox);
  }, 2000)
};

const fiveSecondsDelay = (callback) => {
  return new Promise((resolve) => {
    const loading = document.querySelector('.loading');
    setTimeout(() => {
      loading.remove();
      callback('none');
      resolve('Added books successfully');
    }, 5000);
  });
};
...
const groupAndRenderBooks = (scrollToTop = false) => {
  ...
  if (document.querySelector('.books-group')) {
    ...
    fiveSecondsDelay(renderGroupedBooks);
  }
  scrollToTop && window.scrollTo({ top: 0, behavior: 'smooth' });
};

const createAndAppendALoader = () => {
