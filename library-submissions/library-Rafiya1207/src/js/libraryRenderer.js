...
const addEventsToActionItems = (select, button) => {
...
const delayPage = () => {
  ...
  dropDown.addEventListener('change', delay);
  button.addEventListener('click', delay);
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      dropDown.value = dropDownOptions[0].value;
      dropDown.removeEventListener('change', delay);
      button.removeEventListener('click', delay);
      addEventsToActionItems(dropDown, button);
      resolve();
    }, 5000)
  })
};
...
const render = () => {
  ...
  delayPage()
    .then(() => {
      const booksContainer = document.getElementById('books');
      booksContainer.removeAttribute('class');
      groupAndRenderBooks();
    });
};
