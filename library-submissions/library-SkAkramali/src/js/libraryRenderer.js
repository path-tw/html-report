...
const handleAddBookSubmission = () => {
  ...
  const publishedYearInputElement = document.getElementById('new-book-published-year');
...
const createDropdown = (text, scrollToTop, parent) => {
  return new Promise((resolve) =>{
    ...
    select.id = id;
    select.name = id;
    select.addEventListener('change', loadEventHandler);
...
const createAndAppendAddBookButton = (parent) => {
  return new Promise((resolve) =>{
    ...
    const addBookButton = createButton(text, className, loadEventHandler);
...
const renderBook = (book, parent) => {
  ...
  bookContainer.append(statusDot, bookName, bookAuthor, bookPublishedYear,bookGenre);
...
const addEventToAction = () => {
  const dropDown = document.querySelector('#group-by');
    dropDown.removeEventListener('change', loadEventHandler);
    dropDown.addEventListener('change', () => groupAndRenderBooks(true));
    const addBook = document.querySelector('.add-book-button');
    addBook.removeEventListener('click', loadEventHandler)
    addBook.addEventListener('click', showAddBookPopup);
};

const loadEventHandler = () =>{
  const popupContainer = document.createElement('div');
  const messageContainer = document.createElement('h2');
  messageContainer.innerText = 'BOOKS Are Loading .....';
...
const render = () => {
  addHomePageActions().then((container) =>{
    return  createDropdown('Group', true, container);
  })
  .then((container) =>{
