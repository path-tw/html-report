...
const popupError = () => {
  ...
  popup.className = 'poupup';
  const popuptext = document.createElement('h1');
  popuptext.className = 'popuptext';
  popuptext.innerText = 'Please wait books are still loading...';
  popup.appendChild(popuptext);
  main.appendChild(popup);
  closePopupError(popup);
};
...
const closePopupError = (popup) => {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      popup.classList.add('closePopupError')
      resolve('popup Error');
    }, popUpShowTime)
  })
};
...
const showPopup = popup => {
  if (booksLoaded === 'loading completed') {
