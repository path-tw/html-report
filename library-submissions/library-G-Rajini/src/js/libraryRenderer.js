...
const createAndAppendAddBookButton = parent => {
  ...
  const addBookButton = createButton(text, className, async () => {
    if (!loadResult) {
      createNavLoadingPopup()
    } else {
      showAddBookPopup();
    }
  });
...
const createNavLoadingPopup = () => {
  const loadingPopup = document.createElement('div');
  loadingPopup.className = 'displayLoading';
  loadingPopup.innerText = 'please wait some time the books are loading...';
...
const createLoadingPopup = () => {
  const loadingPopup = document.createElement('div');
  loadingPopup.className = 'display-loading';
