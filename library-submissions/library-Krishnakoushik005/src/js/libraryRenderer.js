...
const showStillLoading = function () {
  const main = document.querySelector('main');
  const div = document.createElement('div');
  div.innerText = 'Book still Loading...';
  main.append(div);
  setTimeout(() => {
    div.remove();
  }, 2000);
...
const createDropdown = (text, scrollToTop, parent) => {
  ...
  select.addEventListener('change', () => {
    const boolOfLoader = document.querySelector('#books').innerText.includes('Loading books...');
    if (boolOfLoader) {
      showStillLoading();
      select.value = 'none';
    } else {
...
const render = () => {  
  addHomePageActions();
  const booksContainer = document.getElementById('books');
  booksContainer.innerText = 'Loading books...';
  setTimeout(() => {
    booksContainer.innerHTML = '';
    groupAndRenderBooks();
  }, 5000);
