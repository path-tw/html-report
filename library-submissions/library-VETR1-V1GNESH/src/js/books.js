...
const enableAddBookFunction = function () {
  const addButton = document.getElementsByClassName('add-book-button')[0];
  addButton.removeEventListener('click', loadingError);
  addButton.addEventListener('click', showAddBookPopup);
};

const enableDropDownFunction = function () {
  const dropDown = document.getElementById('group-by');
  dropDown.removeEventListener('change', loadingError);
...
const main = () => {
  ...
  render().then(() => {
    groupAndRenderBooks();
    enableAddBookFunction();
    enableDropDownFunction();
    removeLoader();
  });
};
