...
const createDropdown = (text, scrollToTop, parent) => {
  ...
  const pop = (isPageLoading) ? popUpLoadingAlert() : groupAndRenderBooks(scrollToTop);
  });
...
const popUpLoadingAlert = () => {
  const pop = document.createElement('h2');
  pop.style.textAlign = 'center';
  pop.innerText = 'Loading is still in process';
...
const createAndAppendAddBookButton = parent => {
  ...
  const addBookButton = createButton(text, className, () => {
    const popUp = (isPageLoading) ? popUpLoadingAlert() : showAddBookPopup();
  });
...
const popUpAlert = () => {
  ...
  popUpDiv.id = 'popUp';
  popUpDiv.innerText = 'Page is still Loading...';
  document.body.appendChild(popUpDiv);
  return;
};

const removePopUp = () => {
  const removeDiv = document.getElementById('popUp');
...
const render = () => {
  ...
  setTimeout (() => {groupAndRenderBooks();}, 5000);
  isPageLoading = false;
  });
};