...
const createLoadingAlert = () => {
  ...
  const main = document.getElementById('main');
  Div.innerText = `Please Wait a second :: Page Is Loading`;
  Div.classList.add('loadingAlert');
  main.append(Div);
  setTimeout(() => {
    Div.remove();
  }, 2000);
...
const setEventForLoadingAlert = () => {
  const dropDown = document.getElementById('group-by');
  const button = document.getElementsByClassName('add-book-button')[0];
  button.addEventListener('click', createLoadingAlert);
  dropDown.addEventListener('click', createLoadingAlert);
};

const removeEvents = () => {
  const dropDown = document.getElementById('group-by');
  const button = document.getElementsByClassName('add-book-button')[0];
  dropDown.removeEventListener('click', createLoadingAlert);
  button.removeEventListener('click', createLoadingAlert);
...
const displayPageState = () => {
  ...
  loadContainer.classList.add('loadPageState');
  mainContainer.appendChild(loadContainer);
  setEventForLoadingAlert();
