...
const createDropdown = (text, scrollToTop, parent) => {
  ...
  select.id = id;
  select.name = id;
  select.addEventListener('change', () => torender);
...
const createAndAppendAddBookButton = parent => {
  ...
  const addBookButton = createButton(text, className, () => showBook);
...
const displayPopup = () => {
  ...
  displayMessage.classList.add('popup');
  displayMessage.innerText = 'Page Still is Loading';
  displayMessage.id = 'displayPopup';
  
  document.body.appendChild(displayMessage);

  setTimeout(() => {
      displayMessage.style.display = 'none';
      document.body.removeChild(displayMessage);
  }, 2000);
  }
...
const toDisplayPopup = () => displayPopup();
const torender = () => groupAndRenderBooks();
const showBook = () => showAddBookPopup()

const showLoading = () => {
  const container = document.getElementById('books');
  container.classList.add('loading');
  container.innerText = 'Loading...';
  const hide = document.getElementsByClassName('add-book-button')[0];
  const dropDown = document.getElementById('group-by');
  hide.removeEventListener('click', showBook);
  dropDown.removeEventListener('change',torender);
  hide.addEventListener('click',toDisplayPopup);
  dropDown.addEventListener('change',toDisplayPopup);
}
...
const hideLoading = () => {
  ...
  const hide = document.getElementsByClassName('add-book-button')[0];
  const dropDown = document.getElementById('group-by');
  hide.removeEventListener('click', toDisplayPopup);
  dropDown.removeEventListener('change', toDisplayPopup);
  hide.addEventListener('click', showBook);
  dropDown.addEventListener('change', groupAndRenderBooks);
};
