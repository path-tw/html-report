...
const showAddBookPopup = () => {
  ...
  document.querySelector('.books-group') && showPopup(popup);
};
...
const createPopUpWhileBuffering = () => {
  const waitingPopUp = createPopup(
    'Books are loading \n Please Wait...', [], ''
  );
  waitingPopUp.querySelector('.action-button').remove();
  showPopup(waitingPopUp);
  setTimeout(() => {
    waitingPopUp.querySelector('.close-button').click();    
  }, 2000);
};

const waitMessage = (event) => {
  if (event.target.className !== 'home-actions') {
    createPopUpWhileBuffering();
  }
};
...
const loader = () => {
  ...
  const loadBox = createPTag('', 'loading-box');
  loadBox.append(createPTag('', 'loading-progress'));
  container.append(loadBox);
  document.querySelector('#main').append(container);
};

const buffer = () => {
  document.querySelector('.home-actions')
    .addEventListener('click', waitMessage);
  loader();

  return new Promise((resolve) => {
    setTimeout(() => {
      document.querySelector('.home-actions')
        .removeEventListener('click', waitMessage);
