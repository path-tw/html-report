...
const toggleInitialPopup = (isLoaderRendered, loaderContainer) => {
  const visibleStatus = isLoaderRendered ? 'none' : 'block';
  loaderContainer.style.cssText = `display: ${visibleStatus}`;
};

const manageToggleAlertPopup = () => {
  const loaderContainer = document.querySelector('.alert-container');
  toggleInitialPopup(false, loaderContainer);
  setTimeout(() => {
    toggleInitialPopup(true, loaderContainer);
  }, 2000);
};

const createInitialPopup = (textMessage, className) => {
  const loaderContainer = document.createElement('div');
  loaderContainer.classList.add(className, 'initial-popups');
  const loadingMessage = document.createElement('span');
  loadingMessage.classList.add('loader-pop-message');
...
const preventDefaultAndloadAlert = (userEvent) => {
  userEvent.preventDefault();
  const loaderContainer = createInitialPopup('Still books are loading, please hold on for a while', 'alert-container');
  toggleInitialPopup(true, loaderContainer);
  manageToggleAlertPopup();
};
...
const createDropdown = (text, scrollToTop, parent) => {
  ...
  select.id = id;
  select.name = id;
  select.addEventListener('mousedown', preventDefaultAndloadAlert);
...
const createAndAppendAddBookButton = (parent) => {
  ...
  const addBookButton = createButton(text, className, preventDefaultAndloadAlert);
...
const changeActionsForHomeActions = () => {
  const addBookButton = document.querySelector('.add-book-button');
  const selectFilter = document.querySelector('#group-by');
  selectFilter.removeEventListener('mousedown', preventDefaultAndloadAlert);
  selectFilter.addEventListener('change', () => groupAndRenderBooks(true));
  addBookButton.removeEventListener('click', preventDefaultAndloadAlert);
  addBookButton.addEventListener('click', showAddBookPopup);
};

const clearInitialPopups = (loaderContainer) => {
  toggleInitialPopup(true, loaderContainer);
  if (loaderContainer = document.querySelector('.alert-container')) {
    toggleInitialPopup(true, loaderContainer);
  }
};
