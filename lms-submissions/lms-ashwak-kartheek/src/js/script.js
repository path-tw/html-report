...
The Wheel of Time: The Shadow Rising,1992,Fantasy,Robert Jordan`;
  ...
  function renderBooks(books) {
    ...
    for (let i = 0; i < books.length; i++) {
      ...
      bookElement.dataset.name = book.name;

      const strong = document.createElement("strong");
      strong.textContent = book.name;
      ...
      availability.style.marginBottom = "-30px";
      const statusSpan = document.createElement("p");
      statusSpan.textContent = book.available ? "Available" : "Unavailable";
      statusSpan.classList.add(book.available ? "available" : "unavailable");
      availability.appendChild(statusSpan);

      const lendBtn = document.createElement("button");
      lendBtn.textContent = book.available ? "Lend" : "Return";
      lendBtn.classList.add("toggle-btn", "lend-btn");
      lendBtn.classList.add(book.available ? "available" : "unavailable");

      lendBtn.onclick = function () {
        toggleAvailability(book.name);
      };
      details.appendChild(year);
      details.appendChild(genre);
      details.appendChild(author);
      bookElement.appendChild(strong);
      bookElement.appendChild(details);
      details.appendChild(availability);
      bookElement.appendChild(lendBtn);
  ...
  function toggleAvailability(name) {
    for (let i = 0; i < bookArray.length; i++) {
      if (bookArray[i].name === name) {
        bookArray[i].available = !bookArray[i].available;
        break;
      }
    }
  ...
  sortByGenreBtn.onclick = function () {
    bookArray = sortBooks("genre");
    renderBooks(bookArray);
  };

  sortByAuthorBtn.onclick = function () {
    bookArray = sortBooks("author");
    renderBooks(bookArray);
  };
