const givenData = `The Lord of the Rings: The Two Towers,1954,Fantasy,J.R.R. Tolkien
...
const submitForm = function () {
  ...
  const genre = document.getElementById("typeofgenre");
  const pubYear = document.getElementById("year");
  const submitButton = document.getElementById("submit");
  const form = document.querySelector(".form");
  submitButton.addEventListener("click", () => {
    if (bookName.value !== "" && author.value !== "" && genre.value !== "" && pubYear.value !== "") {
      const box = document.createElement("div");
      const container = addBooktocategory(genre);
      box.classList.add("box");
      box.appendChild(generateBookName(bookName.value));
      box.appendChild(generateAuthorName(author.value));
      box.appendChild(generateGenreName(genre.value));
      box.appendChild(generatePublishedYear(pubYear.value));
      const buttons = generateButton();
      box.appendChild(buttons[0]);
      box.appendChild(buttons[1]);
      container.appendChild(box);
      form.style.display = "none";
    }
  });
};
const addBooktocategory = function (genre) {
  let container = document.getElementById("container-block").children;
  const containers = document.getElementById("container-block");
  const genres = removeDuplicates(2);
  let j = 1;
  for (let i = 0; i < genres.length - 1; i++,j++) {
    
    if (genre.value === genres[i]) {
      return container[i + j];
    }
  }
  const newcontainer = document.createElement("div");
  const newname = document.createElement("div");
  newname.innerText = genre.value;
  newname.classList.add("name");
  newcontainer.classList.add("containers");
  containers.appendChild(newname);
  containers.appendChild(newcontainer);
  return newcontainer;
};

const exitFromForm = function () {
  const form = document.querySelector(".form");
  form.style.display = "none";
};

const generateBookName = function (bookName) {
  const newBooks = document.createElement("h3");
  newBooks.innerText = "NAME: " + bookName;
  return newBooks;
};

const generateAuthorName = function (bookAuthor) {
  const newAuthor = document.createElement("p");
  newAuthor.innerText = "AUTHOR: " + bookAuthor;
  return newAuthor;
};

const generateGenreName = function (bookGenre) {
  const newGenre = document.createElement("p");
  newGenre.classList.add("genre");
  newGenre.innerText = "GENRE: " + bookGenre;
  return newGenre;
};

const generatePublishedYear = function (publishedYear) {
  const newYear = document.createElement("p");
  newYear.innerText = "PUBLISHED YEAR: " + publishedYear;
  return newYear;
};
...
const generateButton = function () {
  ...
  newbutton.innerText = "Lend";
  availability.innerText = "Available";
  showAvailability(newbutton, availability);
  return [newbutton, availability];
};

const showAvailability = function (button, availability) {
  button.addEventListener("click", () => {
    if (button.innerText === "Lend") {
...
const newBook = function () {
  const books = givenData.split("\n");
  for (let i = 0; i < books.length; i++) {
    let infoSplit = books[i].split(",");
    for (let j = 0; j < infoSplit.length; j += 4) {
      const box = document.createElement("div");
      box.classList.add("box");
      const containers = document.getElementById("container");
      box.appendChild(generateBookName(infoSplit[j]));
      box.appendChild(generateAuthorName(infoSplit[j + 3]));
      box.appendChild(generateGenreName(infoSplit[j + 2]));
      box.appendChild(generatePublishedYear(infoSplit[j + 1]));
      const buttons = generateButton();
      box.appendChild(buttons[0]);
      box.appendChild(buttons[1]);
...
const generateAddBookBtn = function () {
  const newbutton = document.createElement("button");
  newbutton.setAttribute("id", "addBook");
...
const categoriesBygenrebtn = function () {
  const categorybutton = document.createElement("button");
  categorybutton.setAttribute("id", "categoriseBtn");
  categorybutton.innerText = "categorise by genre";
...
const createcontainers = function (index) {
  const containers = document.getElementById("container-block");
  let bookProperty = removeDuplicates(index);
  for (i = 0; i < bookProperty.length; i++) {
    const name = document.createElement("div");
    name.innerText = bookProperty[i];
    name.classList.add("name");
    containers.appendChild(name);
    const container = document.createElement("div");
    container.classList.add("container");
    containers.appendChild(container);
    categorise(container, bookProperty[i], index);
  }
};

const categoriesByAuthorbtn = function () {
  const categorybutton = document.createElement("button");
  categorybutton.setAttribute("id", "categoriseBtn");
  categorybutton.innerText = "categorise by Author";
  const mainContainer = document.getElementById("container");
  categorybutton.addEventListener("click", () => {
    mainContainer.style.display = "none";
    const index = 3;
...
const removeDuplicates = function (index) {
  let genre = [];
  const books = givenData.split("\n");
  for (let i = 0; i < books.length; i++) {
    let infoSplit = books[i].split(",");
    for (let j = 0; j < infoSplit.length; j += 4) {
      if (infoSplit[j + index] === undefined) {
        continue;
      }
      genre.push(infoSplit[j + index]);
    }
  }
  uniqueGenre = [...new Set(genre)];
  return uniqueGenre;
};

const generateHomeButton = function () {
  const newbutton = document.createElement("button");
  newbutton.classList.add("home");
  newbutton.innerText = "All books";
  newbutton.addEventListener("click", () => {
    const mainContainer = document.getElementById("container");
...
const categorise = function (container, propertyType, index) {
  const books = givenData.split("\n");
  for (let i = 0; i < books.length; i++) {
    let infoSplit = books[i].split(",");
    for (let j = 0; j < infoSplit.length; j += 4) {
      if (infoSplit[j + index] === propertyType) {
        const box = document.createElement("div");
        box.classList.add("box");
        box.appendChild(generateBookName(infoSplit[j]));
        box.appendChild(generateAuthorName(infoSplit[j + 3]));
...
const addButtonOnNavBar = function () {
  const navBar = document.getElementById("navbar");
  navBar.appendChild(generateHomeButton());
  navBar.appendChild(categoriesBygenrebtn());
  navBar.appendChild(categoriesByAuthorbtn());
  navBar.appendChild(generateAddBookBtn());
};
...
window.onload = () => {
  ...
  newBook();
  addButtonOnNavBar();
  exit.addEventListener("click", exitFromForm);
};
