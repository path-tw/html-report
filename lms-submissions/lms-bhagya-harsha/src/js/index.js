...
const bookDetails =`The Lord of the Rings: The Two Towers,1954,Fantasy,J.R.R. Tolkien
...
function showForm() {
  const displayer = document.getElementById("formContainer");
  displayer.style.display = 'block';
  const background = document.getElementById("mainContainer");
...
if (!title || !year || !genre || !author) {
  alert("You should enter all the input fields!");
  return;
}
  assigning(title,year,genre,author);
    closeForm();
    formDefault();
};
...
const addingBooks = () => {
    let spiltArray= [];
   spiltArray = bookDetails.split('\n');
   for(let key=0; key<spiltArray.length;key++){
    spiltArray[key] = spiltArray[key].split(',');
 evaluvating(spiltArray[key]);
   } 
   
}

const evaluvating = (array) => {
  let bookTitle, bookYear, bookGenre, bookAuthor;
  for(let key = 0 ; key < array.length ; key++){
    if(key == 0){
      bookTitle = array[key];}
        ...
        bookAuthor = array[key];}
    }
assigning(bookTitle,bookYear,bookGenre,bookAuthor);

};
const assigning = (bookTitle,bookYear,bookGenre,bookAuthor) => {
 ...
 const year = document.createElement('p');
  ...
  const takeOrGive = document.createElement('input');
takeOrGive.setAttribute("type","submit");
takeOrGive.setAttribute("id","actionButton");
takeOrGive.setAttribute("value","lend");

 buttonFunctionality(available,takeOrGive,container);
  title.innerText =  bookTitle;
  year.innerText = 'published year : ' + bookYear;
  genre.innerText = 'Genre: ' + bookGenre;
  author.innerText = 'Author : ' + bookAuthor;
  appending(parent,container,title,year,genre,author);
};

const appending = (parent,container,title,year,genre,author) => {
...
const buttonFunctionality =(available,takeOrGive,container)=>{
  container.appendChild(available);
  container.appendChild(takeOrGive);
  takeOrGive.innerText = 'lend';
  available.innerText = 'Available'; 
  takeOrGive.addEventListener('click',()=>{
    if(takeOrGive.value === "lend"){
      takeOrGive.value = "receive";
      available.style.color = "red";
      available.style.borderColor = "red";
      available.style.textDecoration = "line-through";
    } else{
        takeOrGive.value = "lend";
        available.style.color = "green";
        available.style.borderColor = "green";
        available.style.textDecoration = "none";
      }
     });
};

const categorizeBooksByGenre = () => {
  const containers = document.getElementById('container').children;
  const sortedGenre = [];

  for (let index = 0; index < containers.length; index++) {
    const bookContainer = containers[index];
    const genreText = bookContainer.querySelector('.genre').innerText.substring(7);

    let exists = false;
    for (let i = 0; i < sortedGenre.length; i++) {
      if (sortedGenre[i].genreName === genreText) {
        sortedGenre[i].books.push(bookContainer);
        exists = true;  
        break;
      }
    }
    ...
    if (!exists) {
      sortedGenre.push( { genreName: genreText, books: [bookContainer] } );
    }
  }

  sortedGenre.sort((a, b) => a.genreName.localeCompare(b.genreName));

  const originalContainer = document.getElementById('container');
  originalContainer.innerText = '';

  for (let i = 0; i < sortedGenre.length; i++) {
    const genreObject = sortedGenre[i];
    for (let j = 0; j < genreObject.books.length; j++) {
      const bookContainer = genreObject.books[j];
      originalContainer.appendChild(bookContainer);
    }
  }
};

const categorizeBooksByAuthor = () => {
  const containers = document.getElementById('container').children;
  const sortedAuthor = [];

  for (let index = 0; index < containers.length; index++) {
    const bookContainer = containers[index];
    const authorText = bookContainer.querySelector('.author').innerText.substring(8);

    let exists = false;
    for (let i = 0; i < sortedAuthor.length; i++) {
      if (sortedAuthor[i].authorName === authorText) {
        sortedAuthor[i].books.push(bookContainer);
        exists = true;  
        break;
      }
    }
    ...
    if (!exists) {
      sortedAuthor.push( { authorName: authorText, books: [bookContainer] } );
    }
  }

  sortedAuthor.sort((a, b) => a.authorName.localeCompare(b.authorName));

  const originalContainer = document.getElementById('container');
  originalContainer.innerText = '';

  for (let i = 0; i < sortedAuthor.length; i++) {
    const genreObject = sortedAuthor[i];
    for (let j = 0; j < genreObject.books.length; j++) {
      const bookContainer = genreObject.books[j];
      originalContainer.appendChild(bookContainer);
    }
