window.onload = () => {
  document.getElementById('addBook').addEventListener('click', addBookFunction);
  document.getElementById('submit').addEventListener('click', onsubimtFunction);
  allBooksFunction();
  const formInput = document.getElementsByClassName('formInput');
  for (let index = 0; index < formInput.length; index++) {
    formInput[index].addEventListener('input', setInputRequired);
  }
...
const addBookFunction = () => {
  ...
  const library = document.getElementById('books'); //List of books on UI
...
const clearInput = () => {
  ...
  document.getElementById('submit').style = 'none';
};

const onsubimtFunction = () => {
  const library = document.getElementById('books'); //List of books on UI
  const bookContainer = document.createElement('div');
  bookContainer.setAttribute('class', 'outerContainer');
  library.appendChild(bookContainer);
  const bookImg = document.createElement('img');
  bookImg.setAttribute('src', './source/images/defaultbook.png');
  bookImg.setAttribute('class', 'bookImages');
  bookContainer.appendChild(bookImg);
  const inputBookName = document.createElement('p');
  inputBookName.setAttribute('class', 'bookName');
  bookContainer.appendChild(inputBookName);
  inputBookName.innerText = 'Book Name : ' + bookName.value;
  const inputAuthorName = document.createElement('p');
  inputAuthorName.setAttribute('class', 'authorName');
  bookContainer.appendChild(inputAuthorName);
  inputAuthorName.innerText = 'Author : ' + authorName.value;
  const inputGenreAndYearName = document.createElement('p');
  inputGenreAndYearName.setAttribute('class', 'genreYear');
  bookContainer.appendChild(inputGenreAndYearName);
  inputGenreAndYearName.innerText = 'Genre | Published Year : ' + genre.value + '  |  ' + year.value;
  const buttons = document.createElement('div');
  buttons.className = 'buttons';
  bookContainer.appendChild(buttons);
  const inputGetButton = document.createElement('button');
  inputGetButton.setAttribute('class', 'getBook');
  buttons.appendChild(inputGetButton);
  inputGetButton.innerText = 'Get Book';
  inputGetButton.addEventListener('click', () => {
    handleReturnBookClick(inputGetButton, inputReturnButton);
  });
  const inputReturnButton = document.createElement('button');
  inputReturnButton.setAttribute('class', 'returnBook');
  buttons.appendChild(inputReturnButton);
  inputReturnButton.innerText = 'Return Book';
  inputReturnButton.addEventListener('click', () => {
    handleReturnBookClick(inputGetButton, inputReturnButton);
  });
  inputReturnButton.disabled = true;
  newBookForm.style.display = 'none';
  library.style.display = 'block';
  library.style.display = 'flex';
  library.style.flexWrap = 'wrap';
};
//required is not working in html form
const setInputRequired = () => {
  const formInput = document.getElementsByClassName('formInput');
  const submit = document.getElementById('submit');
  const message = document.getElementById('message');
  for (let index = 0; index < formInput.length; index++) {
    formInput[index].setInputRequired = 'true';
...
const allBooksFunction = () => {
  const library = document.getElementById('books'); //List of books on UI
  const newArray = newBooks.split('\n');
  for (let index = 0; index < newArray.length; index++) {
    const myarray = newArray[index].split(',');
    const bookContainer = document.createElement('div');
    bookContainer.setAttribute('class', 'outerContainer');
    library.appendChild(bookContainer);
    const bookImg = document.createElement('img');
    ...
    inputBookName.setAttribute('class', 'bookName');
    bookContainer.appendChild(inputBookName);
    inputBookName.innerText = 'Book Name : ' + myarray[0];
    const inputAuthorName = document.createElement('p');
    inputAuthorName.setAttribute('class', 'authorName');
    bookContainer.appendChild(inputAuthorName);
    inputAuthorName.innerText = 'Author : ' + myarray[3];
    const inputGenreAndYearName = document.createElement('p');
    inputGenreAndYearName.setAttribute('class', 'genreYear');
    bookContainer.appendChild(inputGenreAndYearName);
    inputGenreAndYearName.innerText = 'Genre | Published Year : ' + myarray[2] + '  |  ' + myarray[1];
    const buttons = document.createElement('div');
    buttons.className = 'buttons';
    bookContainer.appendChild(buttons);
    const inputGetButton = document.createElement('button');
    inputGetButton.setAttribute('class', 'getBook');
    buttons.appendChild(inputGetButton);
    inputGetButton.innerText = 'Lend Book';
    inputGetButton.addEventListener('click', () => {
      handleGetBookClick(inputGetButton, inputReturnButton);
    });
    const inputReturnButton = document.createElement('button');
    inputReturnButton.setAttribute('class', 'returnBook');
    inputReturnButton.setAttribute('disabled', true);
    buttons.appendChild(inputReturnButton);
    inputReturnButton.innerText = 'Return Book';
    inputReturnButton.addEventListener('click', () => {
      handleReturnBookClick(inputGetButton, inputReturnButton);
...
const handleGetBookClick = (getButton, returnButton) => {
  getButton.disabled = true;
  getButton.style.backgroundColor = 'white';
  getButton.style.cursor = 'not-allowed';

  returnButton.disabled = false;
  returnButton.style.backgroundColor = 'rgb(239, 116, 116)';
};

const handleReturnBookClick = (getButton, returnButton) => {
  returnButton.disabled = true;
  returnButton.style.backgroundColor = 'white';
  returnButton.style.cursor = 'not-allowed';

  getButton.disabled = false;
  getButton.style.backgroundColor = 'lightgreen';
};

const sortBooks = () => {
  const sortOption = document.getElementById('sortOptions').value;
  const booksContainer = document.getElementById('books');
  const bookElements = Array.from(booksContainer.children);
  bookElements.sort((a, b) => {
    const aGenreElement = a.querySelector('.genreYear');
    const bGenreElement = b.querySelector('.genreYear');
    const aAuthorElement = a.querySelector('.authorName');
    const bAuthorElement = b.querySelector('.authorName');
    const aGenre = aGenreElement.innerText.split(':')[1];
    const bGenre = bGenreElement.innerText.split(':')[1];
    const aAuthor = aAuthorElement.innerText.split(':')[1];
    const bAuthor = bAuthorElement.innerText.split(':')[1];
    if (sortOption === 'genre') {
  ...
  for (let i = 0; i < bookElements.length; i++) {
    booksContainer.appendChild(bookElements[i]);
  }
};

const newBooks = `The Lord of the Rings: The Two Towers,1954,Fantasy,J.R.R. Tolkien
