...
let csvData = `Name,PublishedYear,Genre,Author
...
window.onload = function () {
  ...
  const catagorize = document.getElementById('categoriser');
  const addBook = document.querySelector('#addBook');
  addBook.addEventListener('click', () => {
    let check = formDisplay(form, container, catagorize);
    newBookDetails(container, form, catagorize, check);
  });
  home.addEventListener('click', () => { homeEventHandler(container) });
  dataConverter(container, catagorize);
  catagorizeEvent(catagorize, container);
};
// requirement one
const dataConverter = (container, catagorize) => {
  let bookData = [];
  const line = csvData.split('\n');
  const keys = line[0].split(',');
  for (let index = 1; index < line.length; index++) {
    bookData = line[index].split(',');
    dataIteration(bookData, keys, catagorize, container);
  }
};

const dataIteration = (bookData, keys, catagorize, container) => {
  let fullDataOfBook = [];
  let tempObject;
  for (let counter = 0; counter < bookData.length; counter++) {
    if (keys[counter] === 'Name') {
      tempObject = ` ${bookData[counter]}`;
    } else {
      tempObject = `${keys[counter]} : ${bookData[counter]}`;
    }
    fullDataOfBook[counter] = tempObject;
  }
  loadData(container, fullDataOfBook, keys, catagorize);
};

const loadData = (container, fullDataOfBook, keys, catagorize) => {
  let content = document.createElement('div');
  const availabialityRemainder = document.createElement('span');
  availabialityRemainder.innerText = 'This book is avliable';
  content.className = 'content';
  for (let index = 0; index < fullDataOfBook.length; index++) {
    const dataElement = document.createElement('p');
    dataElement.id = keys[index];
    dataElement.innerText = fullDataOfBook[index];
    content.append(dataElement);
  }
  content.append(availabialityRemainder);
  availabiality(content, availabialityRemainder);
  container.append(content);
};

// requirement two
const newBookDetails = (container, form, catagorize, check) => {
  const name = document.querySelector('#name');
  const publishedYear = document.querySelector('#publishyear');
  const genre = document.querySelector('#genre');
  const author = document.querySelector('#author');
  const submit = document.querySelector('#submitbutton');
  const back = document.querySelector('#backButton');
  back.addEventListener('click', function () {
    backButton(form, container, catagorize);
  });
  submit.addEventListener('click', (event) => {
    validateForm(form, container, name, publishedYear, author, genre, check, event, catagorize);
  });
};

const newDataInserter = (newBook, container, catagorize) => {
  const line = csvData.split('\n');
  const keys = line[0].split(',');
  dataIteration(newBook, keys, catagorize, container);
};

const formDisplay = (form, container, catagorize) => {
  form.style.display = 'block';
  container.style.display = 'none';
  catagorize.disabled = true;
  return true;
};

const backButton = function (form, container, catagorize) {
  form.style.display = 'none';
  container.style.display = 'flex';
  catagorize.disabled = false;
};

const validateForm = (form, container, name, publishedYear, author, genre, check, event, catagorize) => {
  if (check) {
    check = false;
    event.preventDefault();
    if ((name.value === '' || publishedYear.value === '' || author.value === '' || genre.value === '')) {
      alert('Please fill in all fields');
      return;
    }
    sendNewBookData(form, container, name, publishedYear, author, genre, catagorize)
  }
};

const sendNewBookData = (form, container, name, publishedYear, author, genre, catagorize) => {
  const newBook = [name.value, publishedYear.value, genre.value, author.value];
  form.reset();
  catagorize.disabled = false;
  newDataInserter(newBook, container, catagorize);
  container.style.display = 'flex';
  form.style.display = 'none';
};

// requirement three
const availabiality = function (content, availabialityRemainder) {
  const lendButton = document.createElement('button');
  lendButton.className = 'avaialble';
  lendButton.innerText = 'Lend';
  lendButton.addEventListener('click', function () {
    isAvailable(content, availabialityRemainder, lendButton)
  });
  content.append(lendButton);
};

const isAvailable = function (content, availabialityRemainder, lendButton) {
  lendButton.style.display = 'none';
  const returnButton = document.createElement('button');
  returnButton.className = 'return';
  returnButton.innerText = 'Return';
  content.append(returnButton);
  availabialityRemainder.innerText = 'This book is not available';
  availabialityRemainder.style.color = 'red';
  returnButton.addEventListener('click', function () {
    returnButton.style.display = 'none';
    lendButton.style.display = 'block';
    availabialityRemainder.innerText = 'This book is available';
    availabialityRemainder.style.color = 'green';
  });
};

//requrement four
const catagorizeEvent = (catagorize, container) => {
  catagorize.addEventListener('click', () => {
    const catList = document.querySelector('.buttonList');
    catList.style.display = 'block';
    const closeCatList = document.getElementById('closeCatList');
    closeCatList.addEventListener('click', () => {
      catList.style.display = 'none';
      const addBook = document.getElementById('addBook');
      addBook.disabled = false;
    });
    callCatButton(container);
  });
};

const callCatButton = (container) => {
  let temporaryHolder = [];
  const books = document.querySelectorAll('.content');
  const catList = document.querySelector('.buttonList');
  const eventButtons = document.getElementsByClassName('catButtons');
  callButtonsEvent(eventButtons, catList, books, temporaryHolder, container);
};

const callButtonsEvent = (eventButtons, catList, books, temporaryHolder, container) => {
  for (let index = 0; index < eventButtons.length; index++) {
    eventButtons[index].addEventListener('click', () => {
      container.style.display = 'none';
      catList.style.display = 'none';
      const catContent = eventButtons[index].innerText;
      const addBook = document.getElementById('addBook');
      addBook.disabled = true;
      categoriser(books, temporaryHolder, catContent);
    });
  }
};

const homeEventHandler = (container) => {
  const home = document.getElementById('home');
  const uncatContent = document.querySelectorAll('.content');
  for (let index = 0; index < uncatContent.length; index++) {
    container.append(uncatContent[index]);
  }
  container.style.display = 'flex';
  catContainer.style.display = 'none';
  const closeBtnList = document.querySelector('.buttonList');
  closeBtnList.style.display = 'none';
  const addBook = document.getElementById('addBook');
  addBook.disabled = false;
};

const clearPreviousDiv = (catContainer, catContent) => {
  const removeContainer = document.querySelectorAll('.catorisedContainer');
  for (let index = 0; index < removeContainer.length; index++) {
    const array = Array.from(removeContainer[index].children);
    if (array.length === 1) {
      catContainer.removeChild(removeContainer[index]);
    }
  }
};

const categoriser = (books, temporaryHolder, catContent) => {
  for (let index = 0; index < books.length; index++) {
    const catagorizeElement = books[index].querySelector(`${'#'}${catContent}`);
    let temporaryElement = catagorizeElement.innerText.replace(`${catContent + ' : '}`, '');
    if (temporaryHolder[index] !== temporaryElement) {
      temporaryHolder[index] = temporaryElement;
    }
  }
  temporaryHolder = [...new Set(temporaryHolder)].sort();
  createCatorisedDiv(temporaryHolder, books, catContent);
};

const createCatorisedDiv = (temporaryHolder, books, catContent) => {
  for (let index = 0; index < temporaryHolder.length; index++) {
    const div = document.createElement('div');
    const genreHeading = document.createElement('h1');
    genreHeading.textContent = temporaryHolder[index];
    div.id = temporaryHolder[index];
    div.append(genreHeading);
    div.classList.add('catorisedContainer', `${catContent}`);
    div.style.display = 'flex';
    const catContainer = document.getElementById('catContainer');
    catContainer.style.display = 'flex';
    catContainer.append(div);
  }
  sendCatoriseData(books, catContent, catContainer);
};

const sendCatoriseData = (books, catContent, catContainer) => {
  for (let index = 0; index < books.length; index++) {
    catorisedData(books[index], catContent);
  }
  clearPreviousDiv(catContainer, catContent);
};

const catorisedData = (book, catContent) => {
  const catagorizeElement = book.querySelector(`${'#'}${catContent}`);
  let temporaryElement = catagorizeElement.innerText.replace(`${catContent + ' : '}`, '');
  const GenreContainer = document.getElementById(temporaryElement);
  GenreContainer.appendChild(book);
};
