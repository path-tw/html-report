const addCheckBox = function (section) {
  const checkBox = document.createElement('input');
  const checkBook = document.createElement('span');
  checkBox.setAttribute('class', 'checkBox');
  checkBox.setAttribute('type', 'checkbox');
  checkBook.setAttribute('class', 'checkBook');
  checkBook.innerText = 'available';
  checkBox.checked = 'true';
  section.className =  'book';
  section.appendChild(checkBox);
  section.appendChild(checkBook);
}
 
const createAndAppendElements = function (section, arrayOfObject, header) {   
  ...
  section.appendChild(publishYear);
  section.appendChild(genre);
  section.appendChild(author);
  let index = 0;
  bookName.innerText = arrayOfObject [header[index++]];
  publishYear.innerText = 'Published Year :' + arrayOfObject [header[index++]];
  genre.innerText = 'Genre :' + arrayOfObject [header[index++]];
  author.innerText = 'Author :' + arrayOfObject [header[index++]];
}
   
const createLendAndTake = function (section) {
  const lendAndTake = document.createElement('span');
  section.appendChild(lendAndTake);
  lendAndTake.innerText = 'TakeBook';
  lendAndTake.className = 'lendAndTake';
}
 
const workOfCheckBox = function (lendAndTake) {
  lendAndTake.addEventListener('click', ()=>{
    const book = lendAndTake.parentElement;
    const checkBook = book.querySelector('.checkBook');
    const checkBox = book.querySelector('.checkBox');
    if(checkBox.checked === true) {
      checkBox.checked = false;
      checkBook.innerText = "unavailable";
      lendAndTake.innerText = 'LendBook';
      book.style.backgroundColor = 'rgba(140, 185, 234, 0.532)';
    } else {
      checkBox.checked = true;
      checkBook.innerText = "available";
      lendAndTake.innerText = 'TakeBook';
...
const checkAvailableOrNot = function () {
  const totalLendAndTake = document.querySelectorAll('.lendAndTake');
     for (let index = 0; index < totalLendAndTake.length; index++) {
      workOfCheckBox(totalLendAndTake[index]);
    }
 }


const catgorise = function (event) {
  const genre = event.target.value;
  const section = document.querySelectorAll('.book');
  for (let index = 0; index < section.length; index++) {
    const eachGenre = section[index].querySelectorAll('p');
    if(eachGenre[1].innerText == ('Genre :'+genre)) {
...
const addGenre = function (objectGenre) {
  const select = document.querySelector('#genres');
  const options = document.querySelectorAll('.genre');
  let count = 0;
  for (let index = 0; index < options.length; index++) {
    if (objectGenre === options[index].innerText) {
      count = 1;
    }
  }
  if (!count) {
   const option = document.createElement('option');
    option.innerText = objectGenre;
...
const addBook = function () {
  const bookDeatils = document.querySelector('.newBook');
  const body = document.querySelector('#body');
  if (bookDeatils.style.display === 'block') {
    bookDeatils.style.display = 'none';
    body.style.backgroundColor = 'white';
  } else {
    bookDeatils.style.display = 'block';
    body.style.backgroundColor = 'rgba(140, 185, 234, 0.532)';
  }
  checkAvailableOrNot();
}

const displayBookDetails = function (arrayOfObject, header) {
  const bookDeatils = document.querySelector('.newBook');
  ...
  addCheckBox(section);
  section.className = "book";
  createAndAppendElements(section, arrayOfObject, header);
  createLendAndTake(section);
  checkAvailableOrNot();
  bookDeatils.style.display = 'none';
  body.style.backgroundColor = 'white';
 }

const submitBookDetails = function () {
  const bookName = document.getElementById('bookName');
  const publishYear = document.getElementById('publishYear');
  const genre = document.getElementById('genre');
  const author = document.getElementById('author');
  const arrayOfBookDetails = [bookName.value, publishYear.value, genre.value, author.value];
  let header = `Name,Published Year,Genre,Author`;
  header = header.split(',');
  const arrayOfObject = {};
  for (let index = 0; index < arrayOfBookDetails.length; index++) {
    arrayOfObject[header[index]] = arrayOfBookDetails[index];
  }
  if (bookName.value && publishYear.value && genre.value && author.value) {
    displayBookDetails(arrayOfObject, header); 
    addGenre(arrayOfObject['Genre']);
    bookName.value = publishYear.value = genre.value = author.value = '';
  }  
}

const convertArrayToObject = function (splitOfArray, header, arrayOfObject) {
  for (let index = 0; index < splitOfArray.length; index++) {
    arrayOfObject [header[index]] = splitOfArray[index];
  }
}

const displayBooks = function (arrayOfBooks, arrayOfObject) {
  const header = arrayOfBooks[0].split(',');
  for (let index = 1; index < arrayOfBooks.length; index++) {
    const splitOfArray = arrayOfBooks[index].split(',');
    arrayOfObject[index] = {};
    convertArrayToObject(splitOfArray, header, arrayOfObject[index]);
    const main = document.querySelector('main');
    const section = document.createElement('section'); 
    main.appendChild(section);
    addCheckBox(section);
    createAndAppendElements(section, arrayOfObject[index], header);
    addGenre(arrayOfObject[index]['Genre']);
    createLendAndTake(section);
  } 
  checkAvailableOrNot();
}
...
window.onload = function () { 
  const totalBooks =  `Name,Published Year,Genre,Author
  ...
  const arrayOfBooks = totalBooks.split('\n');
  const arrayOfObject = [];
  displayBooks(arrayOfBooks, arrayOfObject);
  console.log(arrayOfObject);
  const catgorised = document.querySelector('#catgorised');
  catgorised.addEventListener('click',() => convert(arrayOfObject));
 };

const convert = function (arrayOfObject) {
  alert('tthgnbnmn');
  let count = 0;
  const books = document.querySelectorAll('.book');
  for (let index = 0; index < books.length; index++) {
    //console.log(document.getElementById(arrayOfObject[index]['Genre']).id !== null);
    //console.log(arrayOfObject[index+1]);
    const abc = document.getElementById('arrayOfObject[index]["Genre"]');
    console.table(abc);
    if (abc !== null) {
      // books[index].style.display = 'block';
       // console.log('ygjdhf' + books[index-1]);
       abc.appendChild(books[index]);
      count++;
     }
    else { 
      const mainBlock = document.querySelector('main');
      const div = document.createElement('div');
      div.id = arrayOfObject[index+1]['Genre'];
