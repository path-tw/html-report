'use strict';
const categorizeBooksByGenre = function (allBooks) {
  const books = document.querySelectorAll('.bookDetails');
  const genreContainers = {};
  books.forEach((book, index) => {
    const genre = allBooks[index + 1]['Genre'];
    const category = document.querySelector('.categoryContainer');
    if (!genreContainers[genre]) {
      genreContainers[genre] = document.createElement('div');
      genreContainers[genre].classList.add('eachCategoryContainer');
      const title = document.createElement('h2');
      title.classList.add('categoryName');
      title.innerText = genre;
      genreContainers[genre].appendChild(title);
      category.appendChild(genreContainers[genre]);
    }
    genreContainers[genre].appendChild(book);
  });
};


const categorisationByGenre = function () {

  const allBooks = booksData();

  const genreButton = document.querySelector('#categoriseByGenre');
  ...
  genreButton.addEventListener('click', function () {
    ...
    category.classList.add('categoryContainer');

    category.classList.remove('displayNone');

    category.innerText = '';

    categorizeBooksByGenre(allBooks);

  })
};

const categoriesClick = function () {

  const categoriesButton = document.querySelector('#categories');

  categoriesButton.addEventListener('click', function () {
    const categories = document.querySelector('#toggleCategories');
    if (categories.classList.contains('displayNone')) {
      categories.classList.remove('displayNone')
    } else {
      categories.classList.add('displayNone');
    }
  })
};

const generateAvailability = function (parent) {
  const isAvailable = document.createElement('p');
  parent.appendChild(isAvailable);
  isAvailable.classList.add('availability');
  isAvailable.innerText = 'Available';
};
const generateBookDetails = function (parent, book, details) {
  details.forEach((detail) => {
    const newElement = document.createElement('p');
    newElement.classList.add(`${detail.toLowerCase()}`);
    parent.appendChild(newElement);
    detail === 'Name' ? newElement.innerText = `${book[detail]}` : newElement.innerText = `${detail}:${book[detail]}`;
  })
};
const generateAvailabilityButton = function (parent) {
  const availabilityButton = document.createElement('button');
  availabilityButton.classList.add('lendTakeBack');
  parent.appendChild(availabilityButton);
  availabilityButton.innerText = 'Lend';
};

const appendBook = function (book, bookDetail, sectionElement) {
  const division = document.createElement('div');
  division.classList.add('details');
  generateAvailability(division);
  generateBookDetails(division, book, bookDetail);
  sectionElement.appendChild(division);
  generateAvailabilityButton(sectionElement);
};

const createBook = function (book, bookDetail) {

  const sectionElement = document.createElement('section');
  sectionElement.classList.add('bookDetails');
  const booksSpace = document.querySelector('.booksContainer');
  booksSpace.appendChild(sectionElement);
  appendBook(book, bookDetail, sectionElement);
};
const booksData = function () {
  const moreBooks =
    `Name,PublishedYear,Genre,Author
...
The Wheel of Time: The Shadow Rising,1992,Fantasy,Robert Jordan`;
  const separateBook = moreBooks.split('\n');
  const bookDetail = separateBook[0].split(",");
  let booksArray = [];
  booksArray.push(bookDetail);
  for (let index = 1; index < separateBook.length; index++) {
    const individualBook = separateBook[index].split(",");
    const eachBook = {};
    bookDetail.forEach((detail, index) => {
      eachBook[detail] = individualBook[index];
    });
    booksArray.push(eachBook);
  }
  return booksArray;
};

const addingNewBooks = function () {
  const booksArray = booksData();
  for (let index = 1; index < booksArray.length; index++) {
    createBook(booksArray[index], booksArray[0]);
  }
};


const formSubmission = function () {
  const addNewBook = document.getElementById('addingNewBook');
  addNewBook.addEventListener('click', function () {
    const formContent = document.getElementById('form');
    if (formContent.classList.contains('displayNone')) {
...
const submit = function () {
  ...
  submitButton.addEventListener('click', function (event) {
    ...
    const newBookDetails = document.querySelectorAll('.eachDetail');
    const formContent = document.getElementById('form');
    const allBooks = booksData();
    const bookDetail = allBooks[0];
    const eachBook = {};
    bookDetail.forEach((detail, index) => {
      eachBook[detail] = newBookDetails[index].value;
    });
    allBooks.push(eachBook);
    createBook(eachBook, bookDetail);
    formContent.reset();
    availabilityClick();
    categorizeBooksByGenre(allBooks);
  })
...
const availabilityClick = function () {
  const availabilityPress = document.querySelectorAll('.lendTakeBack');
  availabilityPress.forEach(button => {
    button.addEventListener('click', function () {
      const isAvailable = button.parentNode.querySelector('.availability');
      if (isAvailable.classList.contains('unavailable')) {
        isAvailable.classList.remove('unavailable');
        isAvailable.innerText = 'Available';
        button.innerText = 'Lend';
      } else {
        isAvailable.classList.add('unavailable');
        isAvailable.innerText = 'UnAvailable';
        button.innerText = 'Take Back';
      }
    })
  })
}
