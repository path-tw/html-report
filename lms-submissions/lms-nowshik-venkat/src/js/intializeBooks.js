...
const bookCsv = `Name,Published Year,Genre,Author
...
const initializeBooks = () => {
  const finalBook = [];
  const bookEachElement = bookCsv.split('\n');
  const bookHeader = bookEachElement[0].split(',');
  for (let indexOne = 1; indexOne < bookEachElement.length; indexOne++) {
    const bookEachValue = bookEachElement[indexOne].split(',');
    finalBook[indexOne] = {};
    for (let indexTwo = 0; indexTwo < bookEachValue.length; indexTwo++) {
      finalBook[indexOne][bookHeader[indexTwo]] = bookEachValue[indexTwo];
    }
  }
  finalBook.forEach((book) => {
    appendBook(book.Name, book["Published Year"], book.Genre, book.Author);
  });
};