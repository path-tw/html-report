...
const booksRawdetails = [
  `Name,Published Year,Genre,Author
...
const convert = function (booksRawdetails) {
  let booksrecord = [];
  for (let i = 0; i < booksRawdetails.length; i++) {
    booksrecord = booksRawdetails[i].split("\n");
  }
  let headers = booksrecord[0].split(",");
  let books = [];
  for (let i = 1; i < booksrecord.length; i++) {
    let bookData = booksrecord[i].split(",");
    let object = {};
    for (let j = 0; j < bookData.length; j++) {
      object[headers[j]] = bookData[j];
    }
    books.push(object);
  }
  return { headers: headers, data: books };
};

let booksData = convert(booksRawdetails);

const displayAllBooks = function (ClassesNames , avail) {
  let booksData = convert(booksRawdetails);

  for (let bookIndex = 0; bookIndex < booksData.data.length; bookIndex++) {
    let book = booksData.data[bookIndex];
    let headers = booksData.headers;
   
    displayBook(book, ClassesNames, headers , avail);
  }
};



const displayBook = function (book, ClassesNames, headers , avail) {
  ...
  for (let headerIndex = 0; headerIndex < headers.length; headerIndex++) {
    ...
    bookDetails.innerText = (headerName === "Name") ? book[headerName] : headerName + " : " + book[headerName];
    bookContents.appendChild(bookDetails);
  }
  availability(bookContents , avail);
  document.getElementById("bookContainer").appendChild(bookContents);
};
...
window.onload = function () {
  ...
  let avail = 'AVAILABLE';
  displayAllBooks(styleClassesNames , avail);
  addBook(styleClassesNames,booksData , avail);
  getCategories(booksData);
};
