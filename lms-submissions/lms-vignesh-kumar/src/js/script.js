...
window.onload = () => {
  generateBooks(generateBooksArray());
  document.getElementById('categorise-button').addEventListener('click', toggleCategoriseOptions);
  document.getElementById("add-button").addEventListener("click", displaypopup);
  document.getElementById("search-button").addEventListener("click", toggleSearchOptions);
};

const libraryData = `The Lord of the Rings: The Two Towers,1954,Fantasy,J.R.R. Tolkien
...
const generateBooksArray = function () {
  let booksList = libraryData.split('\n');
  let newbook = [];
  const booksArray = [];
...
const generateLendButton = function (book, bookContainer) {
  ...
  lendButton.addEventListener('click', function () {
    bookContainer.classList.toggle('lent-book');
    if (lendButton.textContent === 'Lend Book') {
      lendButton.innerText = 'Restore Book';
      lendButton.style.backgroundColor = 'hsl(0, 87%, 57%)';
    } else {
      lendButton.innerText = 'Lend Book';
      lendButton.style.backgroundColor = 'hsl(211, 87%, 57%)';
    }
  });
  return lendButton;
};

const generateBookAttribute = function (attribute, book) {
  const bookName = document.createElement('p');
  bookName.innerText = `${attribute}: ${book[attribute]}`;
  bookName.classList.add('book-details');
  return bookName;
};

const generateBookImage = function (book) {
  const bookImage = document.createElement('div');
  bookImage.innerText = book.name;
  bookImage.classList.add('book-image');
  return bookImage;
};

const generateDetailsContainer = function (book) {
  const detailsContainer = document.createElement('div');
  detailsContainer.classList.add('details-container');
  return detailsContainer;
};

const generateBookContainer = function (book) {
  const bookContainer = document.createElement('div');
  bookContainer.classList.add('book-container');
  return bookContainer;
};

const appendBook = function (id, book) {
  const bookContainer = generateBookContainer();
  const detailsContainer = generateDetailsContainer();
  bookContainer.append(generateBookImage(book));
  detailsContainer.append(generateBookAttribute('name', book));
  detailsContainer.append(generateBookAttribute('year', book));
  detailsContainer.append(generateBookAttribute('author', book));
  detailsContainer.append(generateBookAttribute('genre', book));
  bookContainer.append(detailsContainer);
  bookContainer.append(generateLendButton(book, bookContainer));
  document.getElementById(id).append(bookContainer);
};

const generateBooks = function (booksArray) {
  booksArray.forEach((book) => {
    appendBook('books-container', book);
  });
};

const displaypopup = function () {
  const subjects = document.getElementById('pop-up');
  const background = document.getElementById('background-blur');
  subjects.classList.toggle('hide');
  subjects.classList.toggle('show-popup');
  background.classList.toggle('background-blur');
};

const genreArray = ['adventure', 'comedy', 'history', 'fantasy'];
const checkGenre = function (genre) {
  if (!genreArray.includes(genre.toLowerCase())) {
    genreArray.push(genre);
    const genreOption = document.createElement('div');
    genreOption.innerText = genre;
    genreOption.classList.add('search-options');
    document.getElementById('search-container').append(genreOption);
  }
};

const validateAddedBook = function () {
  const bookName = document.getElementById('book-name');
  const bookGenre = document.getElementById('book-genre');
  if (bookName.value === '' || bookGenre.value === '') {
    displaypopup();
    window.alert('Fill Book Name and Genre*');
    return false;
  } else {
    return true;
  }
...
const addBook = function () {
  if (validateAddedBook()) {
    ...
    appendBook('books-container', BookDetails);
    checkGenre(BookDetails['genre']);
    displaypopup();
...
const displayGenre = function (id) {
  const resultscontainer = (document.getElementById(
    'searched-genre'
  ).textContent = '');
  const selectBox = document.getElementById(id);
  const selectedGenre = selectBox.textContent;
  const books = generateBooksArray();
  books.forEach((book) => {
    if (book['genre'] === selectedGenre) {
      appendBook('searched-genre', book);
    }
  });
  document.getElementById('results-header').innerText = `Search Results for: ${selectedGenre}`;
  toggleSearchOptions();
  toggleResults();
};

const toggleSearchOptions = function () {
  const dropDown = document.getElementById('search-container');
  dropDown.classList.toggle('hide');
  dropDown.classList.toggle('search-dropdown');
};

const toggleResults = function () {
  const searchResults = document.getElementById('searched-genre');
  searchResults.classList.remove('hide');
  searchResults.classList.add('show-results');
};

const createCategoryContainer = function (category) {
  const container = document.createElement('div');
  container.classList.add('categories-container');
  const containerHeader = document.createElement('h1');
  containerHeader.innerText = category;
  container.append(containerHeader);
  return container;
}

const categorizeByGenre = function () {
  ...
  books.forEach((book) => {
    if (!genreList.includes(book.genre)) {
      ...
      const categoryContainer = createCategoryContainer(book.genre);
      categoryContainer.setAttribute('id', book.genre);
      mainContainer.append(categoryContainer);
    } else {
      appendBook(book.genre, book);
    }
  });
};

const toggleCategoriseOptions = function () {
  const dropDown = document.getElementById('categorise-container');
  dropDown.classList.toggle('hide');
  dropDown.classList.toggle('categorise-dropdown');
}