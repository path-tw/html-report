let allData = `Name,Published Year,Genre,Author
...
const array = [];
const genresArray = [];
let situation = 'random';
const splitString = function (checkValue) {
  const result = allData.split("\n");
  for(let index = 1; index < result.length; index++) {
    const test = result[index];
      const testResult = test.split(',');
      addTwoHundered(testResult[0],testResult[1],testResult[2],testResult[3],checkValue);
  }
};

const addTwoHundered = function (loadName,loadYear,LoadGenre,loadAuthor,checkValue) {
  const newDiv = document.createElement('div');
  newDiv.classList.add('bookInfo');
  const newName = document.createElement('h3');
  const newYear = document.createElement('p');
  const newGenre = document.createElement('p');
  newGenre.setAttribute('class','allGenres');
  const newAuthor = document.createElement('p');
  const stockDetails = document.createElement('div');
  stockDetails.classList.add('lendDetailes');
  const avaliable = document.createElement('div');
  avaliable.classList.add('avalaible');
  const lendbtn = document.createElement('button');
  lendbtn.classList.add('lend');
  lendFeatureTwoHundredBooks(newDiv, newName, newYear, newGenre, newAuthor, loadName, loadYear, loadAuthor,LoadGenre,lendbtn,avaliable,stockDetails,checkValue);
};

const lendFeatureTwoHundredBooks = function (newDiv, newName, newYear, newGenre, newAuthor, loadName, loadYear, loadAuthor, LoadGenre,lendbtn,avaliable,stockDetails,checkValue) {
  lendbtn.addEventListener('click',function () {
    bookStock(avaliable,lendbtn);
  });
  avaliable.innerText = "Avaliable";
  lendbtn.innerText = "Lend";
  stockDetails.appendChild(avaliable);
  stockDetails.appendChild(lendbtn);
  avaliable.classList.add('avaliable');
  lendbtn.classList.add('lend');
  printNewLoad(newDiv, newName, newYear, newGenre, newAuthor, loadName, loadYear, LoadGenre, loadAuthor, stockDetails,checkValue);
};

const bookStock = function (avaliable, lendbtn) {
  if(lendbtn.innerText === 'Lend') {
    avaliable.innerText = 'Unavaliable';
    avaliable.style.color = ' red';
    avaliable.style.backgroundColor = '#cbc6c6';
    lendbtn.style.color = '#77D4FC';
    lendbtn.innerText = 'restock';
    lendbtn.style.backgroundColor = 'black';
  } else {
    avaliable.innerText = 'Avaliable';
    avaliable.style.color = 'black';
    avaliable.style.backgroundColor = 'white';
    lendbtn.style.color = '#77D4FC';
    lendbtn.innerText = 'Lend';
    lendbtn.style.backgroundColor = 'black';
  }
...
const printNewLoad = function (newDiv, newName, newYear, newGenre, newAuthor, loadName, loadYear, LoadGenre, loadAuthor, stockDetails,checkValue) {
  found = false;
 if(checkValue === 0) {
   document.getElementById('Adventure').appendChild(newDiv);
 } else {
  assignGenres(LoadGenre,newDiv,found);
 }
  newName.textContent = loadName;
  newYear.textContent = `Published Year: ${loadYear}`;
  newGenre.textContent = `Genre: ${LoadGenre}`;
  newAuthor.textContent = `Author: ${loadAuthor}`;
  newDiv.appendChild(newName);
  newDiv.appendChild(newYear);
  newDiv.appendChild(newGenre);
  newDiv.appendChild(newAuthor);
  newDiv.appendChild(stockDetails);
};

const categorizeTwoHundredBooks = function(LoadGenre,newDiv) {
  console.log(array);
  let foundGenre = false;
  for(let index = 0; index < array.length; index++) {
    if(array[index] === LoadGenre) {
      document.getElementById(LoadGenre).appendChild(newDiv);
      foundGenre = true;
    }
  }
  if(!foundGenre) {
    let newGenreDiv = document.createElement('div');
    newGenreDiv.setAttribute('id',LoadGenre);
    newGenreDiv.classList.add('newGenreStyles');
    document.body.appendChild(newGenreDiv);
    newGenreDiv.appendChild(newDiv);
    array.push(LoadGenre);
    genresArray.push(newGenreDiv);
...
const addingNewBook = function (inputName, inputPublished, inputGenre, inputAuthor, form) {
  const newDiv = document.createElement('div');
  newDiv.classList.add('bookInfo');
  const newName = document.createElement('h3');
  const newYear = document.createElement('p');
  ...
  const lendbtn = document.createElement('button');
  lendbtn.classList.add('lend');
  lendFeature(lendbtn,avaliable,newDiv, newName, newYear, newGenre, newAuthor, inputName, inputPublished, inputGenre, inputAuthor, form, newDiv, stockDetails)
};
const lendFeature = function (lendbtn,avaliable,newDiv, newName, newYear, newGenre, newAuthor, inputName, inputPublished, inputGenre, inputAuthor, form, newDiv, stockDetails) {
  lendbtn.addEventListener('click',function () {
    bookStock(avaliable,lendbtn);
  });
  avaliable.innerText = "Avaliable";
  ...
  avaliable.classList.add('avaliable');
  lendbtn.classList.add('lend');
  printNewBook(newDiv, newName, newYear, newGenre, newAuthor, inputName, inputPublished, inputGenre, inputAuthor, form, newDiv, stockDetails);
}

const printNewBook = function (newDiv, newName, newYear, newGenre, newAuthor, inputName, inputPublished, inputGenre, inputAuthor, form, newDiv, stockDetails) {
  if(inputName.value && inputPublished.value && inputGenre.value && inputAuthor.value) {
    radomOrCategorize(inputGenre.value,newDiv);
    newName.textContent = inputName.value;
    newYear.textContent = `Published Year: ${inputPublished.value}`;
    newGenre.textContent = `Genre: ${inputGenre.value}`;
    newAuthor.textContent = `Author: ${inputAuthor.value}`;
    allData += '\n' + inputName.value + ', ' + inputPublished.value + ', ' + inputGenre.value + ', ' + inputAuthor.value;
    console.log(allData);
    newDiv.appendChild(newName);
    newDiv.appendChild(newYear);
    newDiv.appendChild(newGenre);
...
const radomOrCategorize = function (LoadGenre,newDiv) {
  let found = false;
  if( situation === 'categorize') {
    assignGenres(LoadGenre,newDiv,found);
  } else {
    document.getElementById('Adventure').appendChild(newDiv);
    array.push(LoadGenre);
...
const assignGenres = function (LoadGenre,newDiv,found) {
  for(let index = 0; index < array.length; index++) {
    if(array[index] === LoadGenre) {
      document.getElementById(LoadGenre).appendChild(newDiv);
      found = true;
  ...
  const displayInputForm = function (form) {
...
const categorizeBooksFunction = function () {
  situation = 'categorize';
  const clearBooks = document.querySelectorAll('.bookInfo');
  for (let index = 0; index < clearBooks.length; index++) {
    clearBooks[index].style.display = 'none';
  }
  const checkValue = 1;
  splitString(checkValue);
}
const hideOptions = function (options) {
...
window.onload = function () {
  ...
  const categorizeBooksGenre = document.querySelector('#categorizeBooksGenre');
  const categorizeBooks = document.getElementById('categorizeBooks');
  const form = document.querySelector('#newBookInputs');
  const inputName = document.querySelector('#bookName');
  const submit = document.querySelector('#submit');
  const inputPublished = document.querySelector('#publishedYear');
  const inputGenre = document.querySelector('#genre');
  const inputAuthor = document.querySelector('#author');
  const checkValue2 = 0;
  splitString(checkValue2);
  categorizeBooks.addEventListener('click', () => {
   hideOptions(options);
  })
  categorizeBooksGenre.addEventListener('click', () => {
    categorizeBooksFunction();
  })
  addBookButton.addEventListener('click', () => {
    displayInputForm(form);
  });
  submit.addEventListener('click', function () {
    addingNewBook(inputName, inputPublished, inputGenre, inputAuthor,form);
  });
};